...
always @(*) begin
    ...
    case(alu_op_i)
      ...
      `ALU_LSL : begin
        ...
        else if (b_i < 32) begin
          result_o = a_i << b_i;
          carry_o = (b_i == 0) ? carry_i : a_i[32-b_i];
        end
        else begin
          result_o = 0;
          carry_o = (b_i == 32) ? a_i[0] : 1'b0;
        end
        overflow_o = overflow_i;
        negative_o = result_o[31];
        zero_o = ~|result_o;
      ...
      `ALU_LSR : begin
        if (b_i == 0) begin
          ...
          result_o = a_i >> b_i;
          carry_o = (b_i == 0) ? carry_i : a_i[b_i-1];
        end
        else begin
          result_o = 0;
          carry_o = (b_i == 32) ? a_i[31] : 1'b0;
        end
        overflow_o = overflow_i;
        negative_o = result_o[31];
        zero_o = ~|result_o;
      ...
      `ALU_ADC : begin
        ...
        overflow_o = (a_i[31] & b_i[31] & ~result_o[31]) | (~a_i[31] & ~b_i[31] & result_o[31]);
        negative_o = result_o[31];
        zero_o = ~|result_o;  
      end
